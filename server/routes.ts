...
export async function registerRoutes(
  ...
  app.get(api.mnSchoolDistricts.list.path, async (_req, res) => {
    const data = await storage.getMnSchoolDistricts();
    res.json(data);
  });
...
async function seedDatabase() {
  ...
  // Seed MN School Districts with Greenhouse Candidates
  const mnDistrictsData = await storage.getMnSchoolDistricts();
  if (mnDistrictsData.length === 0) {
    console.log("Seeding MN school districts data...");
